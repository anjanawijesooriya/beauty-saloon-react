  ...
  return createPortal(
    ...
    >
      <div className="bg-white rounded-2xl shadow-modal w-full max-w-sm p-6">
        ...
        {withReason && (
          <div className="mt-4">
            <label className="block text-xs font-medium text-neutral-600 mb-1.5">{reasonLabel}</label>
            <textarea
              value={reason}
              ...
              rows={3}
              maxLength={500}
              className="w-full px-3 py-2.5 border border-neutral-200 rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand-300 text-neutral-700"
            />
          </div>
        )}
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button
            onClick={() => onConfirm(withReason ? reason || undefined : undefined)}
            disabled={pending}
