  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-modal w-full max-w-md p-6">
        <h2 className="font-semibold text-neutral-900 mb-4">Promote User to Stylist</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1.5">Select User</label>
            <select
              value={userId}
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 border border-neutral-200 rounded-xl text-sm font-medium hover:bg-neutral-50 transition-colors">Cancel</button>
            <button type="submit" disabled={!userId || isPending} className="flex-1 py-2.5 gradient-brand text-white rounded-xl text-sm font-medium hover:opacity-90 disabled:opacity-60 transition-opacity">
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-0.5">{stylists?.length ?? '—'} registered stylists</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-100 bg-neutral-50">
              <th className="text-left px-5 py-3.5 font-medium text-neutral-500">Stylist</th>
              <th className="text-left px-5 py-3.5 font-medium text-neutral-500">Specialties</th>
              <th className="text-left px-5 py-3.5 font-medium text-neutral-500">Rating</th>
              <th className="text-left px-5 py-3.5 font-medium text-neutral-500">Bookings</th>
              <th className="text-left px-5 py-3.5 font-medium text-neutral-500">Status</th>
            </tr>
          </thead>
          <tbody>
            {isLoading
              ...
              : stylists?.map((stylist) => (
                  <tr key={stylist.id} className="border-b border-neutral-50 hover:bg-neutral-50 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-neutral-900">{stylist.user.name}</p>
                          <p className="text-xs text-neutral-400">{stylist.user.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {stylist.specialities.length > 2 && (
                          <span className="text-xs text-neutral-400">+{stylist.specialities.length - 2}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-medium text-neutral-700">{stylist.rating.toFixed(1)}</span>
                        <span className="text-neutral-400 text-xs">({stylist.reviewCount})</span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      ...
                      >
                        {stylist.isAvailable ? <ToggleRight size={14} /> : <ToggleLeft size={14} />}
        ...
        {!isLoading && !stylists?.length && (
          <div className="py-16 text-center text-neutral-400">
            ...
            <button onClick={() => setShowPromote(true)} className="text-brand-400 text-sm hover:underline">Promote a user</button>
          </div>
        )}
      </div>
